...
const ZoraMint = () => {
  ...
  const [currentTab, setCurrentTab] = useState("ERC1155");

  const handleChange = (e) => {
    console.log(e);
    const { name, value } = e.target;

    if (name === "chargeForMintZora") {
      if (value < 0.1) {
        setZoraStatesError({
          ...zoraStatesError,
    ...
    setZoraEnabled((prevEnabled) => ({ ...prevEnabled, [name]: value }));
  };

  // Calendar Functions:
  const onCalChange = (value, dateString, isPresaleSchedule) => {
  ...
  // funtions for adding new input box for royalty split revenue
  const addRecipientInputBox = () => {
    setZoraEnabled({
      ...zoraEnabled,
      royaltySplitRecipients: [
        ...zoraEnabled.royaltySplitRecipients,
        {
          address: "",
          share: null,
        },
  ...
  const removeRecipientInputBox = (index) => {
    const updatedRecipients = zoraEnabled.royaltySplitRecipients.filter(
      (_, i) => i !== index
    );
  ...
  const restrictRemoveRecipientInputBox = (index, recipient) => {
    const isRecipient = parentZoraRoyaltyListRef.current.includes(recipient);
          ...
          <>
            <Tabs className="overflow-y-auto m-2" value={currentTab}>
              <TabsHeader className="relative top-0">
                <Tab value={"ERC1155"} onClick={() => setCurrentTab("ERC1155")}>
                  {" "}
                  ERC1155{" "}
                </Tab>
                <Tab value={"ERC721"} onClick={() => setCurrentTab("ERC721")}>
                  {" "}
                  ERC721{" "}
                </Tab>
              </TabsHeader>
